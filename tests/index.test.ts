...
import { APIPromise } from 'sparkcrm/core/api-promise';

import util from 'node:util';
import SparkCRM from 'sparkcrm';
import { APIUserAbortError } from 'sparkcrm';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['SPARK_CRM_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: SparkCRM) => {
    ...
    test('default logLevel is warn', async () => {
      const client = new SparkCRM({ apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['SPARK_CRM_LOG'] = 'debug';
      const client = new SparkCRM({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['SPARK_CRM_LOG'] = 'not a log level';
      const client = new SparkCRM({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'SPARK_CRM_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new SparkCRM({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['SPARK_CRM_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new SparkCRM({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['SPARK_CRM_BASE_URL'] = 'https://example.com/from_env';
      const client = new SparkCRM({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['SPARK_CRM_BASE_URL'] = ''; // empty
      const client = new SparkCRM({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://gateway.crmspark.com.br');
    });

    test('blank env variable', () => {
      process.env['SPARK_CRM_BASE_URL'] = '  '; // blank
      const client = new SparkCRM({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://gateway.crmspark.com.br');
    });
    ...
    test('in request options overridden by client options', () => {
      const client = new SparkCRM({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['SPARK_CRM_BASE_URL'] = 'http://localhost:5000/env';
      const client = new SparkCRM({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new SparkCRM({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new SparkCRM({ apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['SPARK_API_KEY'] = 'My API Key';
    const client = new SparkCRM();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['SPARK_API_KEY'] = 'another My API Key';
    const client = new SparkCRM({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new SparkCRM({ apiKey: 'My API Key', fetch: testFetch });
